...
var day9Cmd = &cobra.Command{
	Use:   "day9",
	Short: "Advent of Code 2020 - Day9: Encoding Error",
	Long: `
	Advent of Code 2020
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		codes, err := helpers.ReadIntFile(input)
		if err != nil {
			return err
		}

		const PREAMBLE int = 25
		const WEAKNESS int = 1398413738

		for i := PREAMBLE; i < len(codes); i++ {
			backtrack(i-PREAMBLE, i, codes, codes[i])
		}
...
func backtrack(start int, end int, codes []int, target int) {
	good := 0
	for i := start; i < end; i++ {
		for j := start; j < end; j++ {
			if target == codes[i]+codes[j] {
				good = target
			}
		}
	}
...
func hack(codes []int, weakness int) {
	sum := 0
